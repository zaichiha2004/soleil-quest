...
{screen==="ikigai"&&(
  <div key={animKey} style={{paddingTop:40}}>
    ...
    {(()=>{
      const tipData = {
  passion: {title:L("Passion","Страсть","Pasión"), desc:L("Love + Good At. You could do this forever and you're skilled at it — but without serving what the world needs, it can feel self-indulgent or purposeless.","Любовь + Умение. Ты можешь делать это вечно и хорошо в этом — но без связи с тем, что нужно миру, это может ощущаться бессмысленным.","Amor + Bueno en ello. Podrías hacer esto para siempre y eres hábil — pero sin servir lo que el mundo necesita, puede sentirse sin propósito."), gap:L("Without 'What the world needs' → Satisfaction, but feeling of uselessness","Без 'Что нужно миру' → Удовлетворение, но ощущение бесполезности","Sin 'Lo que el mundo necesita' → Satisfacción, pero sensación de inutilidad")},
  mission: {title:L("Mission","Миссия","Misión"), desc:L("Love + World Needs. You care deeply and your work matters — but without a way to sustain yourself financially, passion alone will exhaust you.","Любовь + Потребности мира. Ты искренне заботишься и твоя работа важна — но без финансовой устойчивости одна страсть тебя истощит.","Amor + Lo que el mundo necesita. Te importa profundamente y tu trabajo importa — pero sin sustento económico, la pasión sola te agotará."), gap:L("Without 'What you can be paid for' → Delight and fullness, but no wealth","Без 'За что могут платить' → Радость и полнота, но без богатства","Sin 'Por lo que puedes ser pagado' → Deleite y plenitud, pero sin riqueza")},
  profession: {title:L("Profession","Профессия","Profesión"), desc:L("Good At + Paid For. You're skilled and financially supported — but if you don't love it or feel it serves a deeper purpose, it will slowly hollow you out.","Умение + Оплата. Ты мастер своего дела и получаешь за это — но если ты не любишь это или не чувствуешь глубокого смысла, это будет медленно опустошать тебя.","Bueno en ello + Pagado. Eres hábil y tienes sustento económico — pero si no lo amas o no sientes que sirve a un propósito más profundo, te vaciará lentamente."), gap:L("Without 'What you love' → Comfortable, but feeling of emptiness","Без 'Что ты любишь' → Комфорт, но ощущение пустоты","Sin 'Lo que amas' → Cómodo, pero sensación de vacío")},
  vocation: {title:L("Vocation","Призвание","Vocación"), desc:L("World Needs + Paid For. You're doing useful work and earning — but without natural skill or genuine love for it, it can feel uncertain and draining over time.","Потребности мира + Оплата. Ты делаешь полезное дело и зарабатываешь — но без природного таланта или настоящей любви к этому, со временем это может ощущаться нестабильным и истощающим.","Lo que el mundo necesita + Pagado. Haces un trabajo útil y ganas — pero sin habilidad natural o amor genuino por ello, puede sentirse incierto y agotador con el tiempo."), gap:L("Without 'What you're good at' → Excitement and complacency, but sense of uncertainty","Без 'В чём ты хорош' → Воодушевление и самодовольство, но ощущение неопределённости","Sin 'En lo que eres bueno' → Emoción y complacencia, pero sensación de incertidumbre")},
};
  ...
  <svg style={{position:"absolute",inset:0,width:"100%",height:"100%",pointerEvents:"none",zIndex:20}} viewBox="0 0 360 440">
    {(()=>{
      ...
      return(<>
        <path d={wp(40,42,138,310)} stroke="rgba(255,255,255,.35)" strokeWidth="1.2" fill="none" strokeDasharray="3,3"/>
<circle cx="138" cy="310" r="2.5" fill="rgba(255,255,255,.5)"/>
<path d={wp(320,42,180,268)} stroke="rgba(255,255,255,.35)" strokeWidth="1.2" fill="none" strokeDasharray="3,3"/>
<circle cx="180" cy="268" r="2.5" fill="rgba(255,255,255,.5)"/>
<path d={wp(40,398,180,352)} stroke="rgba(255,255,255,.35)" strokeWidth="1.2" fill="none" strokeDasharray="3,3"/>
<circle cx="180" cy="352" r="2.5" fill="rgba(255,255,255,.5)"/>
<path d={wp(320,398,262,310)} stroke="rgba(255,255,255,.35)" strokeWidth="1.2" fill="none" strokeDasharray="3,3"/>
<circle cx="262" cy="310" r="2.5" fill="rgba(255,255,255,.5)"/>
      </>);
    })()}
  </svg>
